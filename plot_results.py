...
def percentile_plots_seperate_colorbar(plot_var, i_source):
    ...
    plot_settings = plot_var_settings[plot_var+"_perc"]["plot_handling"]
    plot_settings = plot_settings.get(i_source, plot_settings)

    contour_fill_levels = plot_settings["contour_fill_levels"]
    assert len(contour_fill_levels) == 3 and len(contour_fill_levels[0]) > 0,\
        "contour_fill_levels should be a list with 3 items"
    contour_line_levels = plot_settings.get("contour_line_levels", contour_fill_levels)

    colorbar_ticks = plot_settings.get("colorbar_ticks", contour_fill_levels)
    colorbar_label = plot_var_settings[plot_var+"_perc"]["color_label"]
    ...
    if 'colorbar_tick_fmt' in plot_settings:
        row_item['colorbar_tick_fmt'] = plot_settings["colorbar_tick_fmt"]


    # Determine file name used for saving the plot.
    file_name = plot_var_settings[plot_var+"_perc"]["file_name"]
...
def percentile_plots(plot_var, i_source, file_name=None):
    column_titles = ["5th percentile", "32nd percentile", "50th percentile"]
    plot_var_suffix = ["_perc5", "_perc32", "_perc50"]
    ...
    contour_line_levels = plot_settings.get("contour_line_levels", 3*[contour_fill_levels])
...
def percentile_plots_ref(plot_var, i_source, plot_var_ref, i_source_ref, plot_settings_rel, file_name=None):
    column_titles = ["5th percentile", "32nd percentile", "50th percentile"]
    ...
    row_items[0]['colorbar_label'] = plot_var_settings[plot_var+"_perc"]["color_label"]
    row_items[0]['contour_fill_levels'] = contour_fill_levels
    if 'colorbar_tick_fmt' in plot_settings:
        row_items[0]['colorbar_tick_fmt'] = plot_settings["colorbar_tick_fmt"]
    row_items[0]['contour_line_label_fmt'] = '%.1f'

    row_items[1]['colorbar_ticks'] = colorbar_ticks_rel
    row_items[1]['colorbar_label'] = "Improvement factor [-]"
...
def fixed_height_plot():
    ...
    percentile_plots("v_wind_fixed", 0, file_name=file_name)
...
def baseline_comparison_plot():
    ...
    percentile_plots_ref("v_wind_max", 1, "v_wind_fixed", 0, plot_settings_relative_row, file_name=file_name)
...
def baseline_power_plot():
    ...
    plot_item0 = {
        ...
        'colorbar_tick_fmt': '{:.2f}',
        'colorbar_label': 'Available power [$kW/m^2$]',
    }
    plot_item1 = {
...
def baseline_power_ratio_plot():
    ...
    plot_item0 = {
        ...
        'colorbar_tick_fmt': '{:.1f}',
        'colorbar_label': 'Improvement factor [-]',
        'extend': 'max',
    }
    plot_item1 = {
...
def availability_plot():
    ...
    plot_item10 = {
        ...
        'colorbar_tick_fmt': '{:.0f}',
        'colorbar_label': 'Availability improvement [%]',
    }
    plot_item11 = {
        'data': (100.-nc.variables["p_wind_max_rank300"][height_ceiling_id, :, :])-
...
def availability_plot_multiple_ceilings():
    ...
    plot_item10 = {
        ...
        'colorbar_tick_fmt': '{:.0f}',
        'colorbar_label': 'Availability decline [%]',
    }
...
def availability_40W_m2_comparison(height_ceiling=500.):
    ...
    plot_item2 = {
        ...
        'colorbar_tick_fmt': '{:.0f}',
        'colorbar_label': 'Improvement [days/year]',
        'extend': 'max',
    }
